...
export const withinTolerance = (
  actual: number,
  expected: number,
  absoluteTolerance = PRIMITIVE_ABSOLUTE_TOLERANCE,
  relativeTolerance = PRIMITIVE_RELATIVE_TOLERANCE,
): boolean =>
  Math.abs(actual - expected) <=
  absoluteTolerance + relativeTolerance * Math.max(1, Math.abs(actual), Math.abs(expected));
